...
def image_scaling(img, label, imu):
    ...
    scale = tf.random.uniform([1], minval=0.5, maxval=1.5, dtype=tf.float32)
    ...
    imu = tf.image.resize(tf.expand_dims(imu, 0), new_shape, method='nearest')
...
def read_labeled_image_list(data_dir, data_list):
    ...
    with open(data_list, 'r') as f:
        ...
        for line in f:
            ...
            gt_masks.append(os.path.join(data_dir, gt_mask))
            imu_masks.append(os.path.join(data_dir, imu_mask))
...
class ImageReader(object):
    ...
    def __init__(self, data_dir, data_list, input_size, 
        ...
        self.dataset = tf.data.Dataset.from_tensor_slices((self.images, self.labels, self.imus))
        self.dataset = self.dataset.map(lambda img, lbl, imu: read_images_from_disk((img, lbl, imu), 
                                                                                    self.input_size, random_scale, 
                                                                                    random_mirror, ignore_label, img_mean))
        self.dataset = self.dataset.batch(1)  # or the desired batch size
    ...
    def dequeue(self, num_elements):
        ...
        image_batch, label_batch, imu_batch = next(iterator)
