...
def prepare_label(input_batch, new_size, num_classes, one_hot=True):
    ...
    with tf.name_scope('label_encode'):
        input_batch = tf.image.resize(input_batch, new_size, method='nearest')  # Use nearest interpolation.
...
def inv_preprocess(imgs, num_images, img_mean):
    ...
    for i in range(num_images):
        outputs[i] = (imgs[i] + img_mean)[:, :, ::-1].astype(np.uint8)  # Assuming img_mean is in BGR format
    return outputs
